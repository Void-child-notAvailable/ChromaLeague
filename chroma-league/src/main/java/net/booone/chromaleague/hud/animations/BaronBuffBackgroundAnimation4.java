...
public class BaronBuffBackgroundAnimation4 extends AnimatedFrame {
    ...
    public static Set<RzKey> buildBaronArea() {
        ...
        return new RzKeyJoiner()
                .with(new RzKeySelector().withRowOf(RZKEY_Q).withColumnBetween(RZKEY_Q, RZKEY_E))
                .with(new RzKeySelector().withRowOf(RZKEY_A).withColumnBetween(RZKEY_A, RZKEY_D))
                .with(new RzKeySelector().withRowOf(RZKEY_Z).withColumnBetween(RZKEY_Z, RZKEY_X)).join();
    }
}
