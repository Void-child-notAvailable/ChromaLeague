...
public class BaronBuffBackgroundAnimation2 extends AnimatedFrame {
    ...
    public static Set<RzKey> buildBaronArea() {
        ...
        return new RzKeyJoiner()
                .with(new RzKeySelector().withRowOf(RZKEY_Q).withColumnBetween(RZKEY_Q, RZKEY_T))
                .with(new RzKeySelector().withRowOf(RZKEY_A).withColumnBetween(RZKEY_A, RZKEY_G))
                .with(new RzKeySelector().withRowOf(RZKEY_Z).withColumnBetween(RZKEY_Z, RZKEY_V)).join();
    }
}
